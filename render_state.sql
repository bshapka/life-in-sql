...
-- Additional Notes:
--
-- The world is rendered as a single text field called world. The number of records is equal to config['MAX_Y_COORDINATE'],
-- while the length of each field is equal to config['MAX_X_COORDINATE']. If you are unfamiliar with config['SETTING']
...
-- natural numbers in the interval [0, config['MAX_X_COORDINATE']]. The CTE y_dim generates all of the natural numbers
-- in the interval [0, config['MAX_Y_COORDINATE']]. We then generate our coordinate set/plane by cross joining (i.e.
...
-- by taking the x_coordinate and y_coordinate modulo config['MAX_X_COORDINATE'] and config['MAX_Y_COORDINATE'] respectively.
...
-- However, this result is not the familiar xy plane we desire. Instead, this result has (config['MAX_X_COORDINATE'] + 1)
-- * (config['MAX_Y_COORDINATE'] + 1) rows and 1 column. We use the group_concat function as a last step to render the
    ...
    group_concat(
        ...
        when exists (
            select 1 from
                state
            where
                abs(state.x_coordinate % ((select distinct x_dim.upper_limit from x_dim) + 1)) = x_dim.coordinate
                and abs(state.y_coordinate % ((select distinct y_dim.upper_limit from y_dim) + 1)) = y_dim.coordinate
        )
        then
            '*'
        else
